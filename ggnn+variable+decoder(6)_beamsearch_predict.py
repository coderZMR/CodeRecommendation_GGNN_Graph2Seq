...
# 因使用zerorpc进行RPC导致AttributeError: 'MultiRNNCell' object has no attribute '__name__'错误，现对MultiRNNCell进行封装，加入__name__属性
class MultiRNNCell_with_name(tf.nn.rnn_cell.MultiRNNCell):
    def __init__(self, hidden_size, num_layers):
        #self.name = 'MultiRNNCell'
...
class ProgModel(object):
    ...
    def __init__(self, args, training_file_count, valid_file_count):
        ...
        with self.graph.as_default():
            ...
            #restore_file = 'model_best-2019-06-18-10-06-06_18006'
            #if restore_file is not None or not self.is_training:
            if restore_file is not None:
                # self.initialize_model()
                self.restore_model2('model/' + restore_file)
    ...
    def make_model(self):
        ...
        for (internal_id, task_id) in enumerate(self.params['task_ids']):
            with tf.variable_scope("out_layer_task%i" % task_id):
                ...
                with tf.variable_scope("softmax"):
                    ...
                    self.weights['softmax_weights'] = tf.Variable(glorot_init([self.params['softmax_size'], 2277]))
                    self.weights['softmax_biases'] = tf.Variable(np.zeros([2277]).astype(np.float32))
        ...
        # else:
            # self.saver = tf.train.import_meta_graph('model/model_best-2019-06-18-10-06-06_18006.meta')
    ...
    def run_epoch(self, epoch_name: str, data, is_training: bool):
        ...
        while count < file_count:
            tempGraph = 0
            tempAcc = []
            full_path = "/home/x/mydisk/Graph2SequenceNew/" + str(  # TODO 修改路径
                count + index) + ".json"
            # t = time.time()
            filestr = None
            if is_training:
        ...
        with self.graph.as_default():
            ...
            for epoch in range(1, self.params['num_epochs'] + 1):
                ...
                if val_acc < best_val_acc:
                    ...
                    self.save_model2('/Users/lingxiaoxia/Desktop/Graph2SequenceNew/')   # TODO 修改路径
...
class DenseGGNNProgModel(ProgModel):
    def __init__(self, args, training_file_count, valid_file_count):
        ...
        with open('sources/APIVocabulary.txt', 'r') as f:
            self.api2idx = {}
            self.idx2api = {}
        ...
        with open('sources/WholeVocabulary.txt', 'r') as f:
            self.word2idx = {}
            self.idx2word = {}
        ...
        self.variable2index = {}
        self.index2variable = {}
        with open('sources/VariableNameVocabulary.txt', 'r') as f:
            lines = f.readlines()
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.weights['index2vector'] = tf.Variable(dtype=tf.float32,
                                                   initial_value=np.random.uniform(-0.5, 0.5, [2269, e_dim]))
        ...
        self.weights['output2vector'] = tf.Variable(dtype=tf.float32,
                                                    initial_value=np.random.uniform(-0.5, 0.5, [2277, e_dim]))
        ...
        self.weights['variable_index2vector'] = tf.Variable(dtype=tf.float32,
                                                            initial_value=np.random.uniform(-0.5, 0.5, [2072, v_dim]))
        ...
        # Beam Search
        self.dec_init_states = tf.placeholder(tf.float32, [None], name='dec_init_states')
    ...
    def process_raw_graphs(self, raw_data: Sequence[Any], is_training_data: bool, bucket_sizes=None) -> Any:
        ...
        for d in raw_data:
            ...
            for idx, i in enumerate(variables):
                if i == -1:
                    cur_variable_indexes.append(2071)
                else:
    ...
    def make_batch(self, elements):
        ...
        for d in elements:
            ...
            # print(d['variable'])
            variables = [idx for idx in d['variable'][0][:10]] + [2071 for _ in
            ...
            if target_length >= 60:
                ...
                target_val_array.append(2275)
            else:
                target_val_array.append(2275)
            target_length = len(target_val_array)
            output_orders = []
            output_orders.append(2274)
            for i in range(60):
                if i < target_length:
                    target_task_values.append(target_val_array[i])
                else:
                    target_task_values.append(2276)
            for i in range(59):
                if i < target_length:
                    output_orders.append(target_val_array[i])
                else:
                    output_orders.append(2276)
    ...
    def transform_Str2OneOrTwoDimList(self, str):
        str_list = str.split(',')
        OneOrTwoDimList = []
        temp = []
        for ele in str_list:
            if '[[' in ele:
                temp.append(int(ele[2:]))
            elif '[' not in ele and ']' not in ele:
                temp.append(int(ele))
            elif ']' in ele and ']]' not in ele:
                temp.append(int(ele[:len(ele) - 1]))
                OneOrTwoDimList.append(temp)
                temp = []
            elif ']]' in ele:
                temp.append(int(ele[:len(ele) - 2]))
                OneOrTwoDimList.append(temp)
            elif '[' in ele and '[[' not in ele:
                temp.append(int(ele[1:]))
        return OneOrTwoDimList
    ...
    def predict(self, graphRepresent, graphVocab, variableName, single_targets=[[-1]]):
        ...
        for idx, word in eval(graphVocab).items():
            real_idx = self.word2idx.get(word, 2268) # TODO 修改参数
        ...
        for token in tokens:
            if token == '':
                ...
                index = self.variable2index.get(token, 2071) # TODO 修改参数
                if index != 2071 and len(token) != 1:
        ...
        for step, batch in enumerate(one_batch):
            ...
            hyps = [Hypothesis(tokens=[SOS_ID],
                               log_probs=[0.0],
                               state=tuple([encoder_result[:, :] for _ in range(self.params['decoder_num_layers'])])
            ...
            while steps < MAX_DEC_LEN and len(results) < BEAM_SIZE:
                ...
                # 执行一步解码
                results = self.decodeOne(self.sess, latest_tokens, states)
                all_hyps = []
                for (topk_ids, topk_probs, next_state) in results:
            ...
            print(topAPIidSeqList)
            return topAPIidSeqList
        #     length = len(result) - 1
        #     for idx in range(length):
        #         output_apiSeq += " " + (self.idx2api[result[idx + 1]])
        # return output_apiSeq
    def decodeOne(self, sess, latest_tokens, dec_init_states):
        trg_emb = tf.nn.embedding_lookup(self.weights['output2vector'],
                                        self.latest_tokens)
        # tuple([self.dec_init_states[:, :] for _ in range(self.params['decoder_num_layers'])])
        #dec_new = []
        #for id in range(len(self.dec_init_states)):
        #    dec_new.append(tuple([self.dec_init_states[id][:, :] for _ in range(self.params['decoder_num_layers'])]))


        dec_outputs, next_state = self.dec_cell.call(
            state=tuple(self.dec_init_states), inputs=trg_emb)
        ...
        feed = {
            ...
            #self.latest_tokens: np.transpose(np.array([latest_tokens]))
        }
        to_return = {
...
parser = Parser(data_dir='', config_file=None, config=None, restore="model_best-2019-06-18-10-06-06_18006", restrict=None, freeze_graph_model=False)
