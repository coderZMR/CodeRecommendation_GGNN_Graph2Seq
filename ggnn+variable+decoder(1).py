...
class ProgModel(object):
    ...
    def __init__(self, args, training_file_count, valid_file_count):
        ...
        with self.graph.as_default():
            ...
            if restore_file is not None:
                ...
                self.restore_model2('/Users/lingxiaoxia/Desktop/final_models/' + restore_file)
    ...
    def make_model(self):
        ...
        # Decoder部分
        self.placeholders['decoder_hidden_size'] = tf.placeholder(tf.int32, [], name='decoder_hidden_size')
        self.placeholders['decoder_num_layers'] = tf.placeholder(tf.int32, [], name='decoder_num_layers')
        self.placeholders['trg_emb'] = tf.placeholder(tf.int32, None, name='trg_emb')
        ...
        for (internal_id, task_id) in enumerate(self.params['task_ids']):
            with tf.variable_scope("out_layer_task%i" % task_id):
                ...
                with tf.variable_scope("softmax"):
                    ...
                    self.weights['softmax_weights'] = tf.Variable(glorot_init([self.params['softmax_size'], 25559]))
                    self.weights['softmax_biases'] = tf.Variable(np.zeros([25559]).astype(np.float32))
                ...
                label_weights = tf.sequence_mask(
                    self.placeholders['trg_size'], maxlen=tf.shape(self.placeholders['trg_label'])[1], dtype=tf.float32)
                ...
                self.ops['task_loss%i' % task_id] = task_loss
    ...
    def run_epoch(self, epoch_name: str, data, is_training: bool):
        ...
        while count < file_count:
            tempGraph = 0
            tempAcc = []
            full_path = "/home/inspur/Documents/zyf_Project/gated-graph-neural-network-samples/NewestGGNNTrainingData/jsonV3/json" + str(
                count + index) + ".json"
            # t = time.time()
            filestr = None
            if is_training:
        ...
        with self.graph.as_default():
            ...
            for epoch in range(1, self.params['num_epochs'] + 1):
                ...
                if val_acc > best_val_acc:
                    ...
                    self.save_model2('/Users/lingxiaoxia/Desktop/final_models/')
...
class DenseGGNNProgModel(ProgModel):
    def __init__(self, args, training_file_count, valid_file_count):
        ...
        # Decoder部分
        # 设置LSTM单元大小及Decoder的层数
        HIDDEN_SIZE = self.placeholders['decoder_hidden_size']
        NUM_LAYERS = self.placeholders['decoder_num_layers']
        # 定义Decoder所使用的LSTM结构
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.weights['index2vector'] = tf.Variable(dtype=tf.float32,
                                                   initial_value=np.random.uniform(-0.5, 0.5, [36553, e_dim]))
        ...
        self.weights['variable_index2vector'] = tf.Variable(dtype=tf.float32,
                                                            initial_value=np.random.uniform(-0.5, 0.5, [12802, v_dim]))
        ...
        # Decoder部分
        self.placeholders['trg_emb'] = tf.nn.embedding_lookup(self.weights['index2vector'],
    ...
    def gated_regression(self, last_h, regression_gate, regression_transform, softmax_weights, softmax_biases):
        ...
        # Decoder部分
        # 使用dyanmic_rnn构造解码器。
        # 输出的dec_outputs为每一步顶层LSTM的输出。dec_outputs的维度是 [batch_size, max_time, HIDDEN_SIZE]。
        # self.placeholders['trg_emb']是API的embedding
        # self.placeholders['trg_size']是目标API序列的长度
        with tf.variable_scope("decoder"):
            dec_outputs, _ = tf.nn.dynamic_rnn(
                self.dec_cell, self.placeholders['trg_emb'], self.placeholders['trg_size'], initial_state=inte_vectors)
        # 计算解码器每一步的log perplexity。这一步与语言模型代码相同
        output = tf.reshape(dec_outputs, [-1, self.placeholders['decoder_hidden_size']])
    ...
    def process_raw_graphs(self, raw_data: Sequence[Any], is_training_data: bool, bucket_sizes=None) -> Any:
        ...
        for d in raw_data:
            ...
            for idx, i in enumerate(variables):
                if i == -1:
                    cur_variable_indexes.append(12801)
                else:
    ...
    def make_batch(self, elements):
        ...
        for d in elements:
            ...
            # print(d['variable'])
            variables = [idx for idx in d['variable'][0][:10]] + [12801 for _ in
            ...
            for target_val in d['labels']:
                if target_val is None:
                    ...
                    target_val_array = target_val.split(' ')
                    target_task_values.append(target_val_array)
                    target_task_mask.append(len(target_val_array))
    ...
    def make_minibatch_iterator(self, data, is_training: bool):
        ...
        for step in range(len(bucket_at_step)):
            ...
            batch_feed_dict = {
                ...
                # Decoder部分
                self.placeholders['decoder_hidden_size']: 1024, # todo
                self.placeholders['decoder_num_layers']: 2, # todo
                self.placeholders['trg_size']: batch_data['task_masks']
            }
