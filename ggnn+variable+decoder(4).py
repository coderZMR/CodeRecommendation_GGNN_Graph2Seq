...
class ProgModel(object):
    ...
    def __init__(self, args, training_file_count, valid_file_count):
        ...
        with self.graph.as_default():
            ...
            if restore_file is not None or not self.is_training:
                ...
                self.restore_model2('C:\\Users\RuI\\Desktop\\GGNN_Graph2Seq\\model\\' + restore_file)
    ...
    def run_epoch(self, epoch_name: str, data, is_training: bool):
        ...
        while count < file_count:
            tempGraph = 0
            tempAcc = []
            full_path = "/home/x/mydisk/Graph2SequenceNew/" + str(
                count + index) + ".json"
            # t = time.time()
            filestr = None
            if is_training:
        ...
        with self.graph.as_default():
            ...
            for epoch in range(1, self.params['num_epochs'] + 1):
                ...
                if val_acc < best_val_acc:
                    ...
                    self.save_model2('/home/x/mydisk/Graph2SequenceNew/')
...
class DenseGGNNProgModel(ProgModel):
    ...
    def predict(self, graphRepresent, graphVocab, variableName, single_targets=[[-1]]):
        ...
        for idx, word in eval(graphVocab).items():
            real_idx = self.word2idx.get(word, 36552) # TODO 修改参数
        ...
        for token in tokens:
            if token == '':
                ...
                index = self.variable2index.get(token, 12801) # TODO 修改参数
                if index != 12801 and len(token) != 1:
        ...
        for step, batch in enumerate(one_batch):
            ...
            output_apiSeq = ''.join([self.idx2api[idx] for idx in result])
            print(output_apiSeq)
