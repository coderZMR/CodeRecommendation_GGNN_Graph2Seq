...
class ProgModel(object):
    ...
    def __init__(self, args, training_file_count, valid_file_count):
        ...
        with self.graph.as_default():
            ...
            if restore_file is not None:
                ...
                self.restore_model2('/home/inspur/Documents/zyf_Project/Graph2SequenceLarge/model/' + restore_file)
        ...
        for (internal_id, task_id) in enumerate(self.params['task_ids']):
            with tf.variable_scope("out_layer_task%i" % task_id):
                ...
                with tf.variable_scope("softmax"):
                    ...
                    self.weights['softmax_weights'] = tf.Variable(glorot_init([self.params['softmax_size'],36791]))
                    self.weights['softmax_biases'] = tf.Variable(np.zeros([36791]).astype(np.float32))
    ...
    def run_epoch(self, epoch_name: str, data, is_training: bool):
        ...
        while count < file_count:
            tempGraph = 0
            tempAcc = []
            full_path = "/home/inspur/Documents/zyf_Project/Graph2SequenceLarge/json/" + str(
                count + index) + ".json"
            # t = time.time()
            filestr = None
            if is_training:
        ...
        with self.graph.as_default():
            ...
            for epoch in range(1, self.params['num_epochs'] + 1):
                ...
                if val_acc < best_val_acc:
                    ...
                    self.save_model2('/home/inspur/Documents/zyf_Project/Graph2SequenceLarge/model/')
...
class DenseGGNNProgModel(ProgModel):
    ...
    def prepare_specific_graph_model(self) -> None:
        ...
        self.weights['output2vector'] = tf.Variable(dtype=tf.float32,
                                                   initial_value=np.random.uniform(-0.5, 0.5, [36791, e_dim]))
        ...
        for d in elements:
           ...
           # target_task_values.append(target_val_array)
            ...
            if target_length >= 60:
                ...
                target_val_array.append(36789)
            else:
                target_val_array.append(36789)
            target_length = len(target_val_array)
            output_orders = []
            output_orders.append(36788)
            for i in range(60):
                if i < target_length:
                    target_task_values.append(target_val_array[i])
                else:
                    target_task_values.append(36790)
            for i in range(59):
                if i < target_length:
                    output_orders.append(target_val_array[i])
                else:
                    output_orders.append(36790)
...
model = DenseGGNNProgModel(parser,training_file_count=17,valid_file_count=3)
